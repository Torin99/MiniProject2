...
for estimators in [3,10,50,100,300,500,1000,2000]:
    boost = AdaBoostClassifier(n_estimators = estimators, learning_rate = 0.5)
    model = boost.fit(X_train, train_labels)

    y_pred = model.predict(X_test)

    print("Accuracy with number of estimators", estimators, "is:", metrics.accuracy_score(test_labels, y_pred))

for alpha in [0.1,0.5,0.8,1,1.5,2,3]: 
    boost = AdaBoostClassifier(n_estimators = 300, learning_rate = alpha)
    model = boost.fit(X_train, train_labels)

    y_pred = model.predict(X_test)

    print("Accuracy with learning rate:", alpha, "is:", metrics.accuracy_score(test_labels, y_pred))
